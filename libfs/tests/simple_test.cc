...
void io_bench::prepare(void)
{
	...
	ret = mlfs_posix_mkdir((char *)test_dir_prefix, 0777);
...
#ifdef ODIRECT
		if ((fd = mlfs_posix_open((char *)test_file.c_str(), O_RDWR| O_DIRECT, 0666)) < 0)
#else
		if ((fd = mlfs_posix_open((char *)test_file.c_str(), O_RDWR, 0666)) < 0)
#endif
			err(1, "open");
	} else {
		for (unsigned long i = 0; i < BUF_SIZE; i++) 
...
#ifdef ODIRECT
		fd = mlfs_posix_open((char *)test_file.c_str(), O_RDWR | O_CREAT| O_TRUNC | O_DIRECT,
				S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
#else
		fd = mlfs_posix_open((char *)test_file.c_str(), O_RDWR | O_CREAT| O_TRUNC,
				S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
#endif
		if (fd < 0) {
			err(1, "open");
	...
	if (test_type == SEQ_WRITE || test_type == SEQ_WRITE_READ) {
...
#endif
			ret = mlfs_posix_write(fd, (unsigned char *)buf, io_size);
		...
		for (auto it : io_list) {
			...
			*/
			...
			mlfs_posix_lseek(fd, it, SEEK_SET);
			ret = mlfs_posix_write(fd, (unsigned char *)buf, _io_size);
			if (ret != _io_size) {
				printf("write request %u received len %d\n",
						_io_size, ret);
				errx(1, "write");
		...
		for (auto it : io_list) {
            ...
            //read
            if (*op_it == 0) {
                ret = mlfs_posix_read(fd, (unsigned char *)buf, io_size);
				if (ret < 0) 
					errx(1, "read");
            }
            //write
